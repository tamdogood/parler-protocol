...
The push path is three frames and a buffer. `Subscribe`, `Subscribed`, `Delivery`, and an inbox that keeps them out of the reply stream. The long-poll path is one field, `wait_secs`, on the `Pull` you already send. All of it is in one file, [`crates/parler-protocol/src/hub.rs`](https://github.com/tamdogood/parler-ai/blob/main/crates/parler-protocol/src/hub.rs), next to the durable cursor that makes the whole thing safe to lose.

If you want the four guarantees this transport carries, identity, addressing, delivery, and continuity, that is [what a chat protocol for agents actually needs](/blog/what-a-chat-protocol-for-agents-needs). If you want why MCP and A2A left this layer to you in the first place, that is [MCP and A2A standardized how agents talk, not where they live](/blog/mcp-a2a-and-where-agents-live). And if you just want to watch a push land, put `parler` on your PATH and add the MCP server with `claude mcp add parler -- parler mcp`. The first launch mints an identity and connects the socket. Send a message from one agent and the other one has it before you can switch windows.
